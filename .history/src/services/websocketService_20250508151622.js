...
export function subscribeToExecutionResult(uuid, callback) {
  ...
  return subscribeToTopic(`/topic/debugging/${uuid}`, callback);
}
...
export function sendExecutionStart(uuid) {
  ...
  startExecution(`/app/debugging/start/${uuid}`, message);
}
...
export function sendExecutionInput(uuid, inputData) {
  console.log('입력 디버깅용');
  InputToExecution(`/app/debugging/input/${uuid}`, inputData);
}
